'use client';

import { useState } from 'react';
import { useAccount } from 'wagmi';
import { formatUnits } from 'viem';
import { useClaim } from '@/hooks/useClaim';
import { useUserEntries } from '@/hooks/useUserEntries';

interface ClaimPrizeProps {
  poolAddress: `0x${string}`;
  proofsCID: string;
  claimDeadline: number;
}

export function ClaimPrize({ poolAddress, proofsCID, claimDeadline }: ClaimPrizeProps) {
  const { address } = useAccount();
  const { entries: userEntries } = useUserEntries(poolAddress, address);
  const {
    claimables,
    fetching,
    fetchError,
    claim,
    claimingId,
    txHash,
    confirmed,
    waiting,
    claimError,
    reset,
  } = useClaim(poolAddress, proofsCID, address);

  const [now] = useState(() => Date.now() / 1000);
  const deadlinePassed = claimDeadline > 0 && now > claimDeadline;

  if (!proofsCID) return null;

  if (fetching) {
    return (
      <div className="panel-90s p-4">
        <p className="status-claim">Loading prize data from IPFS...</p>
      </div>
    );
  }

  if (fetchError) {
    return (
      <div className="panel-90s p-4">
        <p className="status-error">Failed to load prize data: {fetchError}</p>
      </div>
    );
  }

  if (claimables.length === 0) return null;

  const claimedIds = new Set(
    userEntries.filter((e) => e.isClaimed).map((e) => Number(e.entryId)),
  );

  return (
    <div className="panel-90s p-4 space-y-4">
      <div>
        <h2 className="text-lg status-claim">Claim Your Prizes</h2>
        {!deadlinePassed && claimDeadline > 0 && (
          <p className="text-xs mt-1">
            Claim deadline: {new Date(claimDeadline * 1000).toLocaleString()}
          </p>
        )}
        {deadlinePassed && (
          <p className="text-xs status-error mt-1">
            Claim deadline has passed
          </p>
        )}
      </div>

      <div className="space-y-2">
        {claimables.map((entry) => {
          const alreadyClaimed = claimedIds.has(entry.entryId);
          return (
            <div
              key={entry.entryId}
              className="panel-90s-inset p-3 flex items-center justify-between"
            >
              <div>
                <p className="font-bold">Entry #{entry.entryId}</p>
                <p className="text-xs">
                  Prize: ${formatUnits(entry.prizeAmount, 6)} USDC
                </p>
              </div>
              {alreadyClaimed ? (
                <span className="btn-90s" style={{ cursor: 'default' }}>
                  Already claimed
                </span>
              ) : deadlinePassed ? (
                <span className="text-xs status-error">
                  Deadline passed
                </span>
              ) : (
                <button
                  type="button"
                  onClick={() => claim(entry.entryId, entry.prizeAmount, entry.proof)}
                  disabled={waiting || claimingId !== null}
                  className="btn-90s-primary"
                >
                  {claimingId === entry.entryId && waiting
                    ? 'Claiming...'
                    : claimingId === entry.entryId && confirmed
                      ? 'Claimed!'
                      : 'Claim'}
                </button>
              )}
            </div>
          );
        })}
      </div>

      {txHash && confirmed && (
        <p className="text-xs status-success">
          Claim confirmed! Tx: <code>{txHash}</code>
        </p>
      )}

      {claimError && (
        <div className="panel-90s-inset p-3">
          <p className="text-xs status-error">{claimError}</p>
          <button
            type="button"
            onClick={reset}
            className="text-xs underline mt-1"
          >
            Dismiss
          </button>
        </div>
      )}
    </div>
  );
}
